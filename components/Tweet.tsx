    ...
    <div className="tweet my-4 w-full rounded-lg border border-gray-200 bg-white px-6 py-4 dark:border-gray-800 dark:bg-gray-900">
              ...
              >
                ...
                <div className="absolute bottom-3 left-2 flex items-center gap-1 text-white">
                  <div className="rounded-[4px] bg-black/70 px-1">
                    {Math.floor(m.duration_ms / 1000 / 60)}:
                    {Math.floor(m.duration_ms / 1000) % 60}
                  </div>
                  <div className="rounded-[4px] bg-black/70 px-1">
                    {compactFormatter.format(m?.public_metrics?.view_count)}{' '}
                    views
                  </div>
                </div>
                <button className="absolute top-1/2 left-1/2 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-4 border-white bg-[#1d9bf0]">
                  <svg
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                    className="h-10 w-10 fill-current pl-1 text-white"
                  >
                    <g>
      ...
      <div className="mt-2 flex text-sm !text-gray-700 dark:!text-gray-300">
        ...
        >
          ...
          <span>{compactFormatter.format(public_metrics.reply_count)}</span>
        </a>
        <a
        ...
        >
          ...
          <span>{compactFormatter.format(public_metrics.retweet_count)}</span>
        </a>
        <a
        ...
        >
          ...
          <span>{compactFormatter.format(public_metrics.like_count)}</span>
        </a>
      </div>
    </div>
  );
